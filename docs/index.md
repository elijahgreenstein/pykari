---
title: "Pykari Documentation"
---

Pykari is a simple static site generator written in Python. The name comes from the Japanese word "pikari" (ピカリ), meaning a "flash of light."

# User Guide

- [Installation](user-guide/installation.html)
- [Command Line Interface](user-guide/cli.html)
- [Project Organization](user-guide/organization.html)
- [Markdown](user-guide/markdown.html)
- [Static Files and Templates](user-guide/static.html)
- [Project Configuration](user-guide/configuration.html)

# How-To Guides

- [How to Write a Pykari Template](howto/howto-template.html)
- [How to Configure Your Pykari Project](howto/howto-configure.html)

